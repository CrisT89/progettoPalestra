...
namespace progettoPalestra.Core.HelperService
{
    public class EmailService
    {
        ...
        /// <summary>
        /// Genera un oggetto MailMessage da usare per le mail di cambio password
        /// </summary>
        /// <param name="User">Utente a cui inviare la mail di creazione </param>
        /// <param name="ClearPassword"></param>
        /// <param name="onParallelThread"></param>
        /// <returns></returns>
        public MailMessage ResetPasswordEmail(User User, string ClearPassword, bool onParallelThread = false)
        {
            MailMessage mail = new MailMessage();
            ...
            SetupImageEmail(mail, _TEMPLATE_MAIL_RESET_PASSWORD_IMAGE, out string imageContentID);
        ...
        private void SetupImageEmail(MailMessage mail, string imageFileName, out string imageContentID)
        {
            ...
            if (_configurationService.IsDevelopment == true)
                localImagePath = Path.Combine(Environment.CurrentDirectory, "progettoPalestraClient", "src", "assets", "img", "email", imageFileName);
            else
